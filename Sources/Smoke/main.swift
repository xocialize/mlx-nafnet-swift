...
// nafnet-smoke <image> <out.png> [signage|siddWidth64|goproWidth64|redsWidth64]
// Drives NAFNetRestorePackage through the REAL MLXServeEngine (register → run) — the full Stage-2 path:
// license-gate admission (MIT/MIT), C10 eligibility, engine-constructs-the-package (C13), the restore
// forward. Reports timing + the split footprint (resident floor / activation peak) per the memory
// harness, plus restored RGB gray-stats so a uniform (silent) output is caught.
...
/// Decode a PNG (RGB) and report luminance mean/min/max — a near-uniform result is the classic
    ...
    static func main() async {
        ...
        guard a.count >= 3 else {
            FileHandle.standardError.write(Data(
                "usage: nafnet-smoke <image> <out.png> [signage|siddWidth64|goproWidth64|redsWidth64]\n".utf8))
            exit(2)
        }
        let imagePath = a[1], outPath = a[2]
        let variantArg = a.count > 3 ? a[3] : "signage"
        let variant = NAFNetVariant(rawValue: variantArg) ?? .signage
        do {
            ...
            let config = NAFNetConfiguration(variant: variant)
            ...
            let t0 = Date()
            let id = try await engine.register(NAFNetRestorePackage.registration, configuration: config)
            let tReg = Date().timeIntervalSince(t0)

            let t1 = Date()
            let resp = try await engine.run(ImageRestoreRequest(image: image), package: id)
            let tRun = Date().timeIntervalSince(t1)

            guard let outImage = (resp as? ImageRestoreResponse)?.image else { throw SmokeError.badResponse }
            ...
            print(String(format: "OK %@ → %@ | %dx%d %@ | reg %.2fs run %.2fs | peakGPU %.0f MB | "
                + "luma mean %.3f [%.3f…%.3f]", variantArg, outPath, outImage.width ?? -1,
